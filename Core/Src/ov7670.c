...
uint8_t OV7670_Init(void){
  ...
  //VSY信号(PA0)
	GPIO_InitStruct.Pin = GPIO_PIN_0;				              //设置PA1对应引脚IO编号
  GPIO_InitStruct.Mode =  GPIO_MODE_INPUT; 		          //设置PA1对应引脚IO工作状态为上拉输入
  GPIO_InitStruct.Pull = GPIO_PULLUP;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;		      //设置PA1对应引脚IO操作速度为50MHZ
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);					      //初始化PA1对应引脚IO
  ...
  //WRST信号(PA3)
	GPIO_InitStruct.Pin = GPIO_PIN_3;		                 //设置WRST对应引脚IO编号
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP; 		     //设置WRST对应引脚IO工作状态为推挽输出
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;		     //设置WRST对应引脚IO操作速度为50MHZ
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);					     //初始化WRST对应引脚IO 
	
	//RCLK信号(PA4)
	GPIO_InitStruct.Pin = GPIO_PIN_4;		                 //设置RCLK对应引脚IO编号
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP; 		     //设置RCLK对应引脚IO工作状态为推挽输出
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;		     //设置RCLK对应引脚IO操作速度为50MHZ
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);					     //初始化RCLK对应引脚IO 
	
	//RRST信号(PA5)
	GPIO_InitStruct.Pin = GPIO_PIN_5;		                  //设置WRST对应引脚IO编号
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP; 		      //设置WRST对应引脚IO工作状态为推挽输出
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;		      //设置WRST对应引脚IO操作速度为50MHZ
  HAL_GPIO_Init(GPIOE, &GPIO_InitStruct);					      //初始化WRST对应引脚IO 
  ...
  MX_SCCB_Init();        		                             //初始化OV7670芯片SCCB接口对应IO口  	  
